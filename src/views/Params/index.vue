<template>
  <div class="params-container">
      ...
      <element-table :tableData="tableData" :cellStyle="cellStyle">
        <el-table-column show-overflow-tooltip prop="id" label="规格参数ID" width="100"></el-table-column>
        <el-table-column show-overflow-tooltip prop="itemCatId" label="CID" width="100"></el-table-column>
      ...
      <el-dialog title="规格参数" :visible.sync="dialogParamsVisible">
        ...
        <el-dialog
          width="50%" title="规格参数添加"
          :visible.sync="dialogParamsInnerVisible"
          append-to-body
        >
        <el-button @click="addDomain">添加分组</el-button>
        <el-form :model="dynamicValidateForm" ref="dynamicValidateForm">
            <el-form-item label="规格参数" v-for="(item, index) in dynamicValidateForm.domains" :key="index">
              ...
              <div v-for="(innerItem, innerIndex) in item.children" :key="innerIndex">
                <el-input v-model="innerItem.childValue" style="width: 60%; margin-left: 80px"></el-input>
                <el-button @click="removeParams(index, innerIndex)">删除参数</el-button>
             </div>
            </el-form-item>
        </el-form>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="paramsContentHandle">确定并添加分组内容</el-button>
        </span>
      </el-dialog>  
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    searchDataHandle(search){
      // 参数模糊查询
      api.paramsSearch({search}).then(res=>{
        this.tableData = res.data.result
      })
    },
    ...
    paramsSubimt(){
      api.insertItemParam({
        ...
        // 序列化 JSON.stringify() 方法用于将 JavaScript 值转换为 JSON 字符串
